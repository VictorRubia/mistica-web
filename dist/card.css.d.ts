export declare const actions: string;
export declare const touchable: string;
export declare const cardContentContainer: string;
export declare const touchableContainer: string;
export declare const boxed: string;
export declare const touchableMediaCardOverlay: string;
export declare const circularMediaOverlay: string;
export declare const touchableNakedMediaOverlay: string;
export declare const touchableCardOverlay: string;
export declare const touchableCardOverlayInverse: string;
export declare const touchableCardOverlayMedia: string;
export declare const mediaCard: string;
export declare const mediaCardContent: string;
export declare const mediaCardAsset: string;
export declare const nakedCardContent: string;
export declare const dataCard: string;
export declare const snapCard: string;
export declare const snapCardTouchableHover: string;
export declare const displayCardContainer: string;
export declare const displayCardContent: string;
export declare const displayCardContentWithTopContent: string;
export declare const displayCardContentWithAsset: string;
export declare const displayCardBackground: string;
export declare const displayCardContentWrapper: string;
export declare const displayCardGradient: string;
export declare const posterCardContentWrapper: string;
export declare const vars: {
    aspectRatio: `var(--${string})` | `var(--${string}, ${string})` | `var(--${string}, ${number})`;
    topActionsCount: `var(--${string})` | `var(--${string}, ${string})` | `var(--${string}, ${number})`;
};
export declare const cardContainer: string;
export declare const dataCardTopActionsWithoutIcon: string;
export declare const flexColumn: string;
